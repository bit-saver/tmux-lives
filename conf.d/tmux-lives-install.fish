...
function __tmux_lives_render_fragment --description 'Emit the tmux.conf fragment (categorizer path + switcher key binds)'
    ...
    test -n "$themekey"; and set -a f "bind-key -n $themekey display-popup -B -E -w 52 -h 24 -- fish --no-config $cat theme-picker '#{client_name}'"
    if test -n "$resizekey"
    ...
    if test (count $argv) -eq 0
        ...
        if set -q TMUX; and tmux list-commands 2>/dev/null | grep -q display-popup
            ...
            if test -f $cat
                ...
                tmux display-popup -B -E -w 52 -h 24 -- fish --no-config $cat theme-picker "$client"
                return 0
            end
        end
