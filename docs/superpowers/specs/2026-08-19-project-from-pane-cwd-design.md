...
**Status: SHIPPED**, 2026-08-29, to `feat/project-from-pane-cwd` (commits `5343bf8` the commandeer path pins `$HOME` — first written inside `__tcz_new_general`, moved to `__tcz_commandeer`'s call site by the whole-branch review fix wave, `b646169` `__tmux_lives_new` inherits the invoking cwd; the naming half itself was inherited from an interrupted prior session at `41daabc` and mutation-verified this cycle before either commit). Written 2026-08-19, revised 2026-08-29 to add a second half. Reverses decisions N3 and N9 of the session-naming design (`2026-08-18-session-naming-design.md`), which shipped at `6da151f`. Everything else in that design — the two-layer name/display model, the claim precedence, `gen-N` stability, the ownership guard, the category model — is unchanged.
...
**It must not fork.** Use `test -e` in a loop, never `git rev-parse`. (`-e`, not `-d` — corrected 2026-08-29: in a linked worktree or a submodule `.git` is a regular FILE holding a `gitdir:` pointer, so a `-d` probe misses both and a pane inside one falls back to its deepest directory's own basename, precisely the failure this walk exists to fix. Still a builtin, still no fork.) This project spent a whole cycle removing per-session subprocesses after `pgrep` on macOS routed through a root daemon and burned four cores; do not reintroduce that shape. A handful of stat calls per session per pass is acceptable, a subprocess is not.
...
**The commandeered-springboard path is the exception and pins `$HOME` — at its own call site, not inside `__tcz_new_general`.** `__tcz_commandeer` creates the session a fresh ShellFish springboard tab is bounced onto, and that tab has no invoking cwd worth honouring: its caller on the `client-attached` path is a `run-shell`, which was measured on an isolated `-L` socket to execute at the **tmux server's** cwd, not the pane's. Rocket's server cwd is currently `~/workspace/tmux-lives`, an artifact of where the server was started, so under the naming rule above a commandeered tab would come up named `tmux-lives` until the user `cd`s. `__tcz_commandeer` therefore passes `$HOME` explicitly.

**`__tcz_new_general` itself inherits, and this spec originally said otherwise — corrected 2026-08-29 by the whole-branch review, which found the spec wrong rather than the implementation.** The pin was first written *into* `__tcz_new_general`, justified entirely by the springboard path above. But `__tcz_new_general` has a **second caller**: `__tmux_lives_picker`'s outside-tmux branch (`conf.d/tmux.fish:205`) reaches it through the dispatcher's `new-general` verb, as a plain child of the user's interactive shell — which has a real cwd they chose. Pinning `$HOME` there made that session come up `gen-N` and unnamed, contradicting rule 2's own principle in the direction opposite this design's headline. So the argument is optional: `__tcz_new_general [<dir>]` pins when given a directory and emits no `-c` at all when not, `__tcz_commandeer` passes `$HOME`, and the `new-general` dispatcher case deliberately passes nothing. The pin lands exactly where its justification applies, and `gen-N` sessions created by the commandeer path stay genuinely unnamed until they go somewhere.
...
- `__tcz_new_general` (`functions/tmux-categorize.fish`) — take an optional birth-directory argument; emit `-c` only when given one. `__tcz_commandeer` passes `$HOME`; the `new-general` dispatcher case passes nothing.
...
- `__tcz_new_general` invoked bare, with the caller's cwd set to something other than `$HOME`, produces a session whose `pane_start_path` is that cwd; invoked with an explicit directory, it pins that directory instead.
- **The `$HOME` pin is covered where it actually lives**: drive the real `__tcz_commandeer` from a cwd that is not `$HOME` and read the created session's `pane_start_path`. Headless, `switch-client` can never succeed and `__tcz_commandeer` disposes of the session it created on a failed switch, so shim exactly that one verb to succeed and leave every other tmux call real. This assertion cannot be shown failing against the *pre-fix* code — pre-fix the pin was unconditional, so it passed there for the wrong reason; it is proven instead by mutation, removing the `$HOME` argument from the call site and confirming it fails.
