...
`sd` is the signed hue travel from `__tmux_lives_theme_reldef` (unchanged: `mono 0 · wheat −20 · mint +20 · amber −40 · sage +40 · ember −72 · teal +72 · coral −100`; negative is warm). `phase` is added to the two large areas' hues (`Hbar`, `Htabs`) uniformly. **It does not reach the endcap directly.** At `place=bar` and `place=tabs` the endcap bridges from the *rendered* bar (§2); at `place=cap` the endcap is not a bridge at all — it *is* the anchor, and the bar is solved backward from it (§2). Whether `phase` reaches the endcap therefore depends on which role is pinned to the seed, not on `mode` alone: in `literal` mode at `place=bar` or `place=cap`, the role the endcap's hue comes from — the rendered bar at `place=bar`, the cap itself at `place=cap` — is pinned to the seed, which carries no phase, so the endcap is frozen regardless of how far `phase` turns; at `place=tabs --mode literal` the tabs carry the seed instead, but the bar the endcap bridges from is still derived (it is never the literal-rendered role there), so the endcap follows `phase` normally — this is the entire `chip` tier, including the curated default `amber chip`. In `derived` mode at any placement the anchor role is itself computed rather than pinned to the seed, so it carries `phase` too, and the endcap follows there as well; since the family floor (§2) is looked up at that phase-rotated hue, a large `phase` can cross a family band and change the bar↔cap separation by several degrees. This is a direct consequence of honouring `literal` (§3) — the bridge (or the anchor, at `place=cap`) reads its role's actual rendered hue, and a `literal`-rendered role's hue is phase-blind by construction — not an oversight. It is documented here and pinned by test rather than special-cased in the code.
...
So the endcap always echoes the scheme — which matters on a terminal with no tab bar, where the status bar is the only surface — and never leads it. (See §1 for how `phase` interacts with this: the floor lookup uses `Hbar`, which stays phase-blind only at `place=bar --mode literal`, where `Hbar` is itself the seed-pinned anchor; at `place=tabs`, or in `derived` mode at `place=bar`, `Hbar` is computed rather than pinned and carries `phase`, so the floor can move with it.)
