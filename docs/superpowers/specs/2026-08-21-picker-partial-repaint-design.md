...
The self-heal is a full 12,697-byte repaint of **identical** content, and with no working sync wrapper on tmux 3.3a it may show as a visible sweep about a second after scrolling stops. **Correction (final review): skipping the heal when the frame is byte-identical to `__tcz_pe_prev` cannot be the fix.** In the ordinary browsing case the heal frame *is* byte-identical to `__tcz_pe_prev` — that is precisely what makes it a heal rather than a correction, since nothing the model doesn't already know about has happened. A byte-identity guard would therefore suppress the heal essentially always, deleting the feature rather than tuning it. If the sweep is real, the levers are **cadence**, not content comparison: heal only after the partial-paint count since the last full paint reaches some threshold K, or at most once every N seconds, so a single scroll burst pays for at most one heal instead of one per settle. That is deliberately not built up front — better to see whether it is real than design against a hypothetical.
