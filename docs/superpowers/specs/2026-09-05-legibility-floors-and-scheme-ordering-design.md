...
2. **Nudge pass.** If no swap works, move the colour's OKLCH lightness to `bar ± floor`, preserving hue and chroma, choosing the direction with genuine gamut headroom. **Prefer downward when `bar` is mid-range**: the no-white rule leaves only ~0.003 of chroma margin near L 0.88, so upward nudges there stall against a floor that is already known to be tight.
...
- **Chroma loss is an open risk.** Pushing a colour to an extreme lightness costs chroma — near-white has almost no chroma headroom. There is an unverified claim in this project's captured history that the existing stage-two accepts up to 93% chroma loss. **Treat that as a claim, not a measurement**, and settle it during implementation: report the chroma delta distribution across the 210-combination sweep before and after. If the loss is severe, prefer the downward nudge more aggressively, since the dark end has more chroma headroom than the light end.
