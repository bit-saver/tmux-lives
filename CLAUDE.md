...
**macOS port (spec 2): implemented** — runtime-only persistence (no launchd units; continuum autosave + first-access restore), `/proc`→`ps` detection (`__tcz_pid_comm`/`__tcz_pid_cmdline`), bare cold-start when no server (exposed on demand as `tmux-lives start`). **Pending:** live Mac smoke (categorize as claude, cold-start `tmux-lives start`, reboot-restore). On the Mac: `fisher install bit-saver/tmux-lives` + run `tmux-lives setup` (binds `prefix S` + `Opt+s` by default; use `--prefix-key`/`--switcher-key` to customize).
