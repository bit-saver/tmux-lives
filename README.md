...
Rendering a scheme is real color math, so tmux-lives caches every scheme it renders to disk under `~/.cache/tmux-lives` (or `$XDG_CACHE_HOME/tmux-lives` if you have that set) — the picker's scheme list and `setup theme list` read from that cache, so opening the picker again for a seed you've already used is instant rather than re-rendering every scheme from scratch. The cache notices on its own when the plugin's theme engine has changed and starts fresh rather than ever serving stale colors, so there's nothing to manage by hand; deleting the directory yourself is always safe too — it's only a cache, and tmux-lives rebuilds whatever it needs the next time you open the picker.
