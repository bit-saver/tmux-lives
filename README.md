...
`↑↓` move · `⇞⇟` page · `b` edit seed · `m` more · `z` shake · `⇥` current/off · `a` apply · `⏎` save · `esc` close · `A` auto.

`a` applies a scheme live without saving it, so you can audition; `⏎` commits. Outside edit mode, `esc` reverts whatever you were previewing — including an uncommitted seed change — and closes. `A` toggles auto-apply, on by default: once the cursor settles on a row for about half a second with no further key, that row applies itself live, same as pressing `a`. It's suspended while you're editing the seed, so dragging a channel only ever previews locally and never reaches the real bar — a seed edit is configuration, and only settling the cursor on a row (or an explicit `a`/`⏎`) counts as adoption. A ShellFish/iTerm2 tab's colour now updates the moment you preview, revert, or auto-apply, rather than lagging behind the ~15-second status-bar tick in either direction.
