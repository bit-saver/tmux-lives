...
The picker (`setup theme`, `M-k`, or `M-m k`) shows a ShellFish tab chip when a ShellFish client is attached, a live bar preview, and labeled adjustment/scheme zones with a key legend: `↑↓` scheme · `←→` phase · `o` rotate · `b` seed (opens RGB sliders — `t` inside for typed hex) · `m` more schemes · `a` apply preview (live, unsaved) · `⏎` save · `r` reset knobs · `Esc`/`q` revert and close. Hold shift on a cycling key to step it backward; changed values flash blue for half a second.
