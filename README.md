...
Moving a channel does not redraw only the seed zone — every keypress still rebuilds the whole frame — but the scheme strips and the `current` row are served from a row cache rather than recomputed, since their underlying colours stay on the old seed while you're actively adjusting, so a redraw now costs about 35ms instead of the ~90ms it cost before this branch's caching. About 700ms after the last channel keypress, the whole batch (every visible strip plus the `current` row) regenerates in one pass — a full batch is 310-800ms depending on whether 14 or 35 rows are visible, too slow to pay on every keystroke, so it waits for you to actually stop rather than paying that cost on each press.
