...
The status bar is themed by a gradient map, and each **scheme** now varies more than trim: it places the bar itself at a different hue on your seed's own depth (lightness) — so schemes give you genuinely different bar colors, not just variations dressed around one fixed bar. The endcaps are derived from wherever that bar lands, by a calibrated pairing rule tuned so the dominant bar/cap pair always reads well together. Your seed itself always shows up somewhere: the ShellFish tabs wear it verbatim (in `mono`, where the bar IS the seed, the tabs take the first accent instead), and the `mono` scheme simply IS it — bar = seed, untouched, the one scheme that never resamples. The remaining accents — separators · active · windows — come from a ring of arc samples around your seed, and they're the only roles `o` rotate shuffles; text stays contrast-safe automatically, computed straight off the actual bar color rather than the seed. Default scheme: `mono`.
